...
#define INSMOD      "insmod " MODULE_NAME " > /dev/null 2>&1"
#define RMMOD       "rmmod " MODULE_NAME " > /dev/null 2>&1"
// TODO(ashmrtn): Expand to work with other file system types.
#define FMT_DRIVE   "mkfs -t ext4 " FULL_LV_PATH " > /dev/null 2>&1"
#define FSCK        "fsck -T -t ext4 " FULL_SN_PATH " -- -y > /dev/null 2>&1"
...
int main(int argc, char** argv) {
  ...
  ((destroy_t*)(killer))(test);
  dlclose(handle);
  cout << "Ran " << num_tests_run << " tests.\n\t" << num_passed_tests
    << " tests passed\n\t" << num_failed_tests << " tests failed\n";
  return 0;
