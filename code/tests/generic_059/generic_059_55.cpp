    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int setup() override {
				...
				if ( fallocate( fd_foo , 0 , 0 , 16384) < 0){ 
					 close( fd_foo);
					 return errno;
				}
				if ( WriteDataMmap( fd_foo, 0, 16384) < 0){ 
					close( fd_foo); 
					return errno;
				}
				...
				close(fd_foo_compare); 
				fd_foo_compare = open(foo_compare_path.c_str() , O_RDWR|O_DIRECT|O_SYNC , 0777); 
				if ( fd_foo_compare < 0 ) { 
					close( fd_foo_compare); 
					return errno;
				}

				void* data_foo_compare;
				if (posix_memalign(&data_foo_compare , 4096, 16384 ) < 0) {
					return errno;
				}

				 
				int offset_foo_compare = 0;
				int to_write_foo_compare = 16384 ;
				const char *text_foo_compare  = "abcdefghijklmnopqrstuvwxyz123456";
				while (offset_foo_compare < 16384){
					if (to_write_foo_compare < 32){
						memcpy((char *)data_foo_compare+ offset_foo_compare, text_foo_compare, to_write_foo_compare);
						offset_foo_compare += to_write_foo_compare;
					}
					else {
						memcpy((char *)data_foo_compare+ offset_foo_compare,text_foo_compare, 32);
						offset_foo_compare += 32; 
					} 
				} 

				if ( pwrite( fd_foo_compare, data_foo_compare, 16384, 0) < 0){
					close( fd_foo_compare); 
					return errno;
				}
				...
				if ( fallocate( fd_foo_compare , FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE , 8000 , 4096) < 0){ 
					 close( fd_foo_compare);
					 return errno;
				}
            ...
            virtual int run() override {
				...
				if ( fallocate( fd_foo , FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE , 8000 , 4096) < 0){ 
					 close( fd_foo);
					 return errno;
				}
