...
class Ext4RegressionBugNoMmapSetup: public BaseTestCase {
 public:
  virtual int setup() override {
    ...
    if (WriteData(fd, 0x136dd, 0xdc69) < 0) {
      close(fd);
      return -3;
    }

    if (fallocate(fd, 0, 0xb531, 0xb5ad) < 0) {
      close(fd);
      return -4;
    }

    if (fallocate(fd, FALLOC_FL_COLLAPSE_RANGE, 0x1c000, 0x4000) < 0) {
      close(fd);
      return -4;
    }
...
extern "C" fs_testing::tests::BaseTestCase *test_case_get_instance() {
  return new fs_testing::tests::Ext4RegressionBugNoMmapSetup;
}
