    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_Afoo, FALLOC_FL_ZERO_RANGE, 0, 8192) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				if (checkpoint == local_checkpoint) {
					return 0;
				}

				if ( WriteData ( fd_Afoo, 4096, 4096) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}

				if ( cm_->CmSyncFileRange( fd_Afoo , 4096, 4096, SYNC_FILE_RANGE_WAIT_BEFORE|SYNC_FILE_RANGE_WRITE|SYNC_FILE_RANGE_WAIT_AFTER) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
