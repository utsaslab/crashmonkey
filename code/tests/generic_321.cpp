...
class Generic321_2: public BaseTestCase {
 public:
  ...
  virtual int run() override {
    ...
    //Checkpoint 1 where we fsync the dir - foo should be either in root or dir A. If present in both, it should be removable and also allow dir A to be removed	
    if (Checkpoint() < 0){
      return -5;
    }
    close(dir);
    ...
    // fsync file foo
    int fd_foo_moved = open(foo_moved_path.c_str(), O_RDONLY, TEST_FILE_PERMS);
    if (fd_foo_moved < 0) {
      return -1;
    }
  ...
  virtual int check_test(unsigned int last_checkpoint,
    ...
    // If foo is present in both source and destination directories during rename, then we should be able to unlink it from both directories, and directory A itself should be removable.
	...
	if (remove (foo_path.c_str()) < 0)
		test_result->error_description += " : Cannot remove foo from parent dir";
	if (remove (foo_moved_path.c_str()) < 0)
		test_result->error_description += " : Cannot remove foo from destination dir A";
	if (rmdir(dir_path.c_str()) < 0)
		test_result->error_description += " : Dir A unremovable";
	return 0;
    }
