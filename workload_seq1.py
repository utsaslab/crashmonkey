...
def insertSyncRange(contents, line, index_map, method):
    
    to_insert = '\n\t\t\t\tif ( sync_file_range( fd_' + line.split(' ')[1] + ' , ' + line.split(' ')[2] + ' , ' + line.split(' ')[3] + ' , SYNC_FILE_RANGE_WRITE) < 0){ \n\t\t\t\t\tcm_->CmClose( fd_' + line.split(' ')[1]  +');\n\t\t\t\t\t return errno;\n\t\t\t\t}\n\n'
...
def insertFunctions(line, file, index_map, method):
    with open(file, 'r+') as insert:
        ...
        elif line.split(' ')[0] == 'syncrange':
            if method == 'setup':
                updateSetupMap(index_map, 1)
            else:
                updateRunMap(index_map, 1)
            insertSyncRange(contents, line, index_map, method)
