...
#removed setxattr
OperationSet = ['creat', 'mkdir', 'mknod', 'falloc', 'write', 'dwrite', 'link', 'unlink', 'remove', 'rename', 'symlink', 'removexattr', 'fdatasync']

#We are skipping 041, 336, 342, 343
#The sequences we want to reach to
expected_sequence = []
expected_sync_sequence = []
current_sequence = [('creat', ('foo')), ('fsync', ('A/bar')), ('link' , ('A/foo', 'A/bar')) ]
...
def checkCreatDep(current_sequence, pos):
...
def checkExistsDep(current_sequence, pos):
...
def checkClosed(current_sequence, pos):
    file_name = current_sequence[pos][1]
    if file_name not in FileOptions:
        print 'Invalid param'
...
def checkXattr(current_sequence, pos):
    file_name = current_sequence[pos][1]
    if file_name not in FileOptions:
        print 'Invalid param'
...
def satisyDep(current_sequence, pos):
    ...
    if command == 'creat' or command == 'mknod':
        ...
        checkExistsDep(current_sequence, pos)
        ...
        checkExistsDep(current_sequence, pos)
        checkClosed(current_sequence, pos)
        ...
        checkExistsDep(current_sequence, pos)
        #setxattr
        checkXattr(current_sequence, pos)
